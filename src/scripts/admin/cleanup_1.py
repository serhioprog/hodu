"""
PROD CLEANUP §1 — 14-member Pefkohori mismerged cluster — REAL RUN.
...
src/scripts/admin/cleanup_1_dryrun.py — same logic verbatim, only
DRY_RUN flag and docstring differ. Dry-run output was reviewed by
architect on 2026-05-07 (deltas: +88 feedback / -1 APPROVED /
14 properties detached / §5 contradictory pairs 3 -> 0) before
this real-run was authorized.

Mirrors src/main.py:_dissolve_cluster_with_feedback (line 545).

Run via:
    docker compose exec -T scraper python -m src.scripts.admin.cleanup_1
"""

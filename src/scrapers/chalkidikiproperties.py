"""chalkidikiproperties.gr scraper (Kefalidis Real Estate).

Site: Kefalidis Real Estate (https://chalkidikiproperties.gr)
...
Inventory: small boutique broker, ~9 sale listings ≥€400k in Halkidiki.
"""
...
# Constants
# ============================================================================
_SOURCE_DOMAIN = "chalkidikiproperties.gr"
_BASE_URL = "https://chalkidikiproperties.gr"
_SEARCH_URL = (
    "https://chalkidikiproperties.gr/en/property/search"
...
# Scraper
# ============================================================================
class ChalkidikipropertiesScraper(EnrichmentMixin, BaseScraper):
    """Kefalidis Real Estate scraper (Camoufox-based, PerimeterX bypass)."""

    name = "chalkidikiproperties"
    ...
    async def collect_urls(
        ...
        async with AsyncCamoufox(headless=True, humanize=True, geoip=True) as browser:
            ...
            try:
                ...
                logger.error(f"[{self.source_domain}] search page fetch: {exc!r}")
                return []

            # Get all .property-item card data
            cards = await page.evaluate("""() => {
                ...
                for (const a of document.querySelectorAll('a.property-item')) {
                    ...
                    out.push({
                        url: href.startsWith('http') ? href : ('https://chalkidikiproperties.gr' + href),
            ...
            logger.info(f"[{self.source_domain}] search page: {len(cards)} cards")

            if not cards:
                logger.warning(f"[{self.source_domain}] empty search page — no listings or selector changed")
                return []
    ...
    def _parse_agent(self, parser, extras: Dict[str, Any]) -> None:
        ...
        extras["agent_company"] = "Kefalidis Real Estate"
        extras["agent_email"] = "info@chalkidikiproperties.gr"
